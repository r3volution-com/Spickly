...
function sendAjaxPOST (url, params) {
	var xmlhttp;
	var resp = "";
...
function comment(id, type) {	
	...
	if (type == 3) {
		...
		newdiv.innerHTML="<div id=\"allcomment\"><div id=\"eliminar\" onclick=\"cwindows(deleteNewComment("+id+")\");>X</div> <img src=\""+my_info["prf_img"]+" \" width=\"40px\" height=\"40px\" class=\"prf_img_mini\" > <a href=\"in.php?p=profile&id="+my_info["id"]+"\">"+my_info["name"]+" "+my_info["lname"]+"</a><br> <h5>"+datefull+"</h5>"+text+"</div>";
	} else {
		document.getElementById('new_comment_'+id+'').appendChild(newdiv);	
...
function delcomment (id, type) {
	...
	sendAjax('in_actions.php?a=dcomment&d='+id+'&type='+type);
	deltag('commentspacerlist_'+id);
}
