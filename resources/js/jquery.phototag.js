	...
	$.fn.photoTag = function( options ){
		...
		var createNewTagForm = function( tempTagBox, image, image_id ){
			...
			newTagFormBox.css({
				...
				'left': tempTagBoxPosition.left + 10,
			...
			$('#tempNewTagForm').submit(function(e){
				...
				$.getJSON(options.addTagUrl, $(this).serialize()+"&"+$.param(tag),function(response){
					if(response.result != undefined && !response.result){
						manageError(response);
						return;
					}
		...
		var wrapImage = function( image, image_id ){
			...
			var placeList = options.placeList;
			var canvas = $('<div id="' + options.imageWrapBox.canvasIdPrefix + image_id + '" ></div>');
			var wrapper = $('<div class="' + options.imageWrapBox.cssClass + '" id="' + options.imageWrapBox.idPrefix + image_id +'" ></div>');
			...
			if(options.imageWrapBox.showTagList){
				...
				if (!placeList) $('#' + options.imageWrapBox.canvasIdPrefix + image_id).parent().append(tagList);
				else $('#' + placeList).append(tagList);
			}
		}
