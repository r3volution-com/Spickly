	...
	<div id="sidebar" class="sideblock">
		...
		<div ondrop="drop(this, event)" ondragenter="return false" ondragover="return false">
			...
			<form method="GET" action="in_actions.php?a=event">	
				...
				<div id="inv_events_amigos" style="overflow: auto; padding: 10px;">
					<?php
						$pageid = $_GET["id"];
						...
						if (mysql_num_rows($res9)) {
							if (!$friendArray) echo "Aun no tienes amigos :(";
								...
								$res1 = mysql_query("SELECT member_id_receive FROM members_has_pages WHERE member_id_receive IN (".implode(",", $friendArray).") AND alive=1 AND pages_id='".$pageid."' ORDER BY alive") or die(mysql_error());  
								...
								foreach($friendArray as $user1) {
									...
									echo '<img src="'.$image.'" title="'.$u_row["firstname"].'" draggable="true" ondragstart="drag(this, event)" id="user_'.$u_row["member_id"].'" width="75" height="75" >';
								} 
							} 
				...
				<input type="submit" id="inv" name="inv" class="boton"  style="position: relative; top: 10px;">	
			</form>
		</div>
		<script>
			function drag(persona, pagina) {
				pagina.dataTransfer.setData('Text', persona.id);
			}
			function drop(contenedor, pagina) {
				var id = pagina.dataTransfer.getData('Text');
