...
if (!function_exists("URLtoProfile")) {
	function URLtoProfile($text) {
		if ($matches = preg_match_all('#(?:http\://|http\://www\.|www\.)?spickly\.es\/(?:pr\_|in\.php\?p=profile&id=)([0-9]+)#i', $text, $repeat, PREG_PATTERN_ORDER)) {
			$patterns = $repeat[0];
			$replaces = $repeat[1];
			
			for ($i = 0; $patterns[$i]; $i++) {
				$row = fetchUser($replaces[$i]);
				$text = str_replace($patterns[$i], '<a href="http://spickly.es/pr_'.$row["member_id"].'" target="_blank">@'.$row["firstname"].'</a> ', $text);
			}
        } 
		return($text);
	}
