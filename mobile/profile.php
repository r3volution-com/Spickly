			...
			if ($row["member_id_send"] == $user_row["member_id"]){
				...
				if (mysql_num_rows($res2)) {
					...
					while ($drow = mysql_fetch_array($res2)) {
						...
						if ($u_row["member_id"] == $my_user_id) echo '<div class="delspacer" onclick="cwindows(\'deleteSpacerComment('.$drow['id'].')\');">X</div>'; 
						echo "<img src=".$images." width='40px' height='40px' style='border-radius:5px;' class='prf_img_mini'> <a href='in.php?p=profile&id=".$u_row["member_id"]."'>".$u_row["firstname"]." ".$u_row["lastname"].'</a><br><h5>'.$fecha.'</h5>'.$drow['text'].'</div></div><br>';
					}
